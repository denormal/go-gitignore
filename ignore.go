...
func NewGitIgnore( file string ) ( Ignore , error ) {
    ...
    return NewGitIgnoreFromReader( _fh , _base , nil ) , nil
} // NewGitIgnore()


func NewGitIgnoreFromReader( r      io.Reader           ,
                             base   string              ,
                             errors func ( Error ) bool ) Ignore {
    // extract the patterns from the reader
    _parser     := NewParser( r , base , errors )
    _patterns   := _parser.Parse()
    return &ignore{ _base    :  base     ,
                    _pattern : _patterns }
} // NewGitIgnoreFromReader()
...
func NewGitIgnoreWithCache( file  string ,
    ...
    if _ignore != nil {
        _ignore , _err  = NewGitIgnore( file )
        if _ignore == nil {
